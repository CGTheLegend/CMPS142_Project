...
# dataset = np.array(pd.read_csv('movie_metadata_1.csv', names=COLUMNS, skipinitialspace=True))
data_train = np.array(pd.read_csv('movie_metadata_1.csv', names=COLUMNS, skipinitialspace=True))
data_test = np.array(pd.read_csv('movie_metadata_1.csv', names=COLUMNS, skipinitialspace=True))
...
# prof = np.greater_equal(dataset[:, [8]], 2 * dataset[:, [22]]).astype(int)
# # dataset[PROFIT_COLUMN] = np.greater_equal(data_train["gross"], 2 * data_train["budget"]).astype(int)
# data_train = np.hstack((data_train, prof_train))
# data_test = np.hstack((data_test, prof_test))
# dataset = np.hstack((dataset, prof))
...
# 	data_train[:, [28]], batch_size=100)
# input_fn = tf.contrib.learn.io.numpy_input_fn({"director_facebook_likes":director_facebook_likes, "gross":gross, "genres":genres,
# 	"num_voted_users":num_voted_users, "cast_total_facebook_likes":cast_total_facebook_likes, "plot_keywords":plot_keywords,
# 	"content_rating":content_rating, "budget":budget, "imdb_score":imdb_score, "movie_facebook_likes":movie_facebook_likes}, 
# 	data_train[:, [28]], batch_size=100)
